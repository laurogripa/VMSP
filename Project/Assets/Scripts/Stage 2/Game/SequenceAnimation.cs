...
public class SequenceAnimation : MonoBehaviour
{
    ...
    private Sprite[] LoadSpriteGroup(int dID)
    {
            ...
            switch(gameObject.name)
            {
                case "Blue":
                    ...
                    for(int i = 0; i < 36; i++)
                    {
                        if(i < 9)
                        {
                            result[i] = Resources.Load("Stage 2/" + gameObject.name + "/toRed/0" + (i + 1), typeof(Sprite)) as Sprite;
                        }
                        else
                        {
                            result[i] = Resources.Load("Stage 2/" + gameObject.name + "/toRed/" + (i + 1), typeof(Sprite)) as Sprite;
                        }
                    }
                    break;
                case "Green":
                ...
                case "Red":
                    ...
                    for (int i = 0; i < 32; i++)
                    {
                        if (i < 9)
                        {
                            result[i] = Resources.Load("Stage 2/" + gameObject.name + "/toYellow/0" + (i + 1), typeof(Sprite)) as Sprite;
                        }
                        else
                        {
                            result[i] = Resources.Load("Stage 2/" + gameObject.name + "/toYellow/" + (i + 1), typeof(Sprite)) as Sprite;
                        }
                    }
                    break;
                case "Yellow":
        ...
        else
        {
            ...
            {
                case 0:
                    loadedSprites = Resources.LoadAll("Stage 2/" + gameObject.name + "/toBlue/", typeof(Sprite));
                    break;
                case 1:
                    loadedSprites = Resources.LoadAll("Stage 2/" + gameObject.name + "/toGreen/", typeof(Sprite));
                    break;
                case 2:
                    loadedSprites = Resources.LoadAll("Stage 2/" + gameObject.name + "/toRed/", typeof(Sprite));
                    break;
                case 3:
                    loadedSprites = Resources.LoadAll("Stage 2/" + gameObject.name + "/toYellow/", typeof(Sprite));
                    break;
            }
